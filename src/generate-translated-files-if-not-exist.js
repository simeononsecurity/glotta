...
async function generateTranslatedFilesIfNotExist({ dir, targetLanguageIds, recursive }) {
    assertValidLanguageIds(targetLanguageIds);
    const pathsToEnglishFiles = await getPathsOfFilesInLanguage({ startDir: dir, languageId: LANGUAGE_IDS.en, recursive: recursive });
    console.log(pathsToEnglishFiles);
